...
void SMS4EnCrypt(muint8  * m,muint8  * c,muint8  * key)
{
...
void SMS4DeCrypt(muint8  * m,muint8  * c,muint8  * key)
{
...
void  SMS4_Encrypt(unsigned char *m ,unsigned char * k, unsigned char * e,int  n,int * rn)
{
	int i,j,p,q;
	unsigned char dat0[16],dat1[16];
   ...
   if (p>0)
	 {
		 e[n]=0x80;
		 if (p<15) 	 for(i=1;i<= 15-p;i++) e[n+i]=0x00;
	 }     
   
   for(i=0;i<=q;i++)
   {
	   for(j=0;j<16;j++) dat0[j]=e[i*16+j];

	   SMS4EnCrypt(dat0,dat1,k);

	   for(j=0;j<16;j++) e[i*16+j]=dat1[j];
   }
 ...
 void  SMS4_Decrypt(unsigned char *m ,unsigned char * k, unsigned char * e,int  n,int * rn)
{
	int i,j,p,q;
	unsigned char dat0[16],dat1[16];
...
//	muint8 key[16]={0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0 ,0x0,0x0,0x0,0x00};
	muint8 key[16]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98 ,0x76,0x54,0x32,0x10};
	muint8 m[16]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98 ,0x76,0x54,0x32,0x10};
//	muint8 m[16]={0x00,0x08,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x80,0x00,0x00,0x00 ,0x00,0x00};
	muint8 c[16];
	muint32 rk[32];
