...
//		bEmvErr = (THM_ReadReg(EMVERR)&0x3b);
	...
	//	for(bEmvErr =0;bEmvErr<90;bEmvErr++);
//		globle = temp;
	...
	{ 
	    ...
	    delay_ms1(10);				//wait until send over
	}
    else if (temp & PERR)       
...
{
	if (0 == Modetype)
		GPIO_SetBits(GPIOB,GPIO_Pin_12);    // Set PB1 as STANDBY and set SSN high	
	else
		GPIO_ResetBits(GPIOB,GPIO_Pin_12);    // Set PB1 as STANDBY and set SSN LOW	
}
