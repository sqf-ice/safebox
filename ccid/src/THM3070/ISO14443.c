...
unsigned char THM_TestReady(void)
{
//	unsigned char temp,value,ret;
...
void 	ISO14443_carrier_on_off_on(int n,int m)
{
...
{	
	...
	delay_ms1(t_reset);				   //waite treset
	THM_WriteReg(CONTROL_TX,0x72);	   //open RF 
}
 ...
 s16 ISO14443_WUPB(u8 nslot, u8 *ATQB)
{
	...
	if(rvsta&FEND){	
		bSak=  ATQB[10]; 
		if((ATQB[11] & 0x01)==0x01)
			CID_flag1 = 1;
		if((ATQB[0] != 0x50)||(len<12)||((ATQB[10]&0x08) == 0x08))	 
			return ERR_REV_WUPB;
		else if((ATQB[1] == 0)&&(ATQB[2] == 0)&&(ATQB[3] == 0)&&(ATQB[4] == 0) )
		{
		    return 0x95;
		}
		else			
...
unsigned int ISO14443_XfrBlock(unsigned char *senbuf,unsigned int senlen,unsigned char *revbuf,unsigned int relen)
{
...
unsigned char search_typea(unsigned char *rbuf,unsigned int *rlen)
{
 	s16 ret =0;
	unsigned char r_temp[20];
	unsigned char cid,ii;
	cid=0;
	ISO14443_reset(100);
/*	while(1)
	{
		ret=ISO14443_WUPA(r_temp);
		for(cid =0;cid<200;cid++);
		for(cid =0;cid<100;cid++);
		ret=ISO14443_WUPB(1,r_temp);
	} */ 

	for(ii=0;ii<250;ii++);
	ret = ISO14443_Anticollision_TypeA(r_temp);
	if(ret <0)
		return 1;
	ret = ISO14443_RATS(cid,rbuf);
	if(ret <0)
		return 1;
	else
	{
...
unsigned char search_typeb(unsigned char *rbuf,unsigned int *rlen)
{
 	s16 ret =0;
	unsigned char cid =0;
	
	ret = ISO14443_Anticollision_TypeB(cid,rbuf);
	if(ret <0)
	{
		return 1;
	}
...
unsigned char search_card(unsigned char *buf_atr,unsigned int *rlen)
{
	unsigned char ret= 0;

	
	ret = search_typea(buf_atr,rlen);
	if ( ret == 0x00 )
	{
		card_type = 0x0A;
		return 0;							//== 寻卡成功
	}
	else if ( ret == 0x03 )					//== mifare
	{
		card_type = 0x1A;
		return 0;
	}
	else if ( ret == 0x02)					//== 多张卡
	{
		return 0x03;
	}	  

	ret = search_typeb(buf_atr,rlen);
	if(ret == 0x95)
	{
		 card_type = 0xfe;
		 return 0;
...
//===============================================================================//
//== 函 数 名：ISO14443_PowerOn
//== 功    能：非接上电接收ATR
//== 说    明：接收到的数据需要进行封装，封装成标准的CCID的ATR
//== 时    间：
//== 入口参数：	*Atr ATR  buf  ,Atr_len :接收的ATR的长度
//== 出口参数：
//== 返 回 值：
//===============================================================================//

unsigned char ISO14443_PowerOn(unsigned char *Atr,unsigned char *Atr_len)
{
	unsigned char status, i,Tck_xor=0;

    unsigned char buffer[20];
	unsigned int len;

    status = search_card(&buffer[5],&len);	    //	先寻卡
    ...
    { 
	    ...
	    { 
			...
			if(buffer[5]>0x0f)
				buffer[5]=0x0f;
            buffer[1] = buffer[5]; //	
            buffer[1] = 0x80 | ( buffer[1] & 0x0F );
            buffer[2] = 0x80;
			buffer[3] = 0x11;
			buffer[4] = 0xfe;

            *Atr_len = (buffer[5]&0x0f);
            *Atr_len += 5;
//              *Atr_len = 8; 
            memcpy( Atr, buffer, *Atr_len );
			for( i = 1; i < *Atr_len; i++ )
            { //	T0到Tk异或 不包含INIT_HEADER
                Tck_xor ^= buffer[i];
            }
			Atr[*Atr_len] = Tck_xor;
			*Atr_len += 1; 
        ...
        else if((card_type==0x0b)||(card_type==0xfe)) 
        {   
            buffer[0] = 0x3b;
            buffer[1] = 0x6c; //	
            buffer[2] = 0x00;
...
unsigned char Detect_14443_3_Card(void)		
{	
	 s16 error=0;
	 unsigned char temp_buf[15];
//	 ISO14443_reset(10);

	 error = ISO14443_WUPA(temp_buf);
	 if(error>0)
	 {
		 return 0;
	 }
	 error = ISO14443_WUPB(0,temp_buf);
	
	if((error ==0)||(error ==0x95))
	 {
		return 0;
	 }
...
unsigned int rfid_tx_rx(unsigned char* buf, unsigned short *len)
{
	...
	{ 
		...
		{
			if(i%2){
				buf[0] = 0x05;buf[1] = 0x00;buf[2] = 0x08;  					
			}else{
				buf[0] = 0x05;buf[1] = 0x00;buf[2] = 0x00; 		
			}
			THM_SendFrame(buf,3); 	
			ret=THM_WaitReadFrame(len,buf);
