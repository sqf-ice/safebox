...
#include "aS3911.h"
//#include "..\config\CCM3310\hal\config_code.h"
//#include "IS32U320A.h"
 ...
 * asumed.
 */
#define AS3911_RX_IRQ_SANITY_TIMEOUT        2000
...
static volatile bool as3911ReceptionInProgress = FALSE;

/*! \ToDo: Remove this variable. It is no longer used.
 */
static volatile bool as3911TransmissionInProgress = FALSE;
 ...
 *****************************************************************************
 */
static bool_t as3911EmvExceptionProcessing = 0;
 ...
 *****************************************************************************
 */
static bool as3911FdtIsRunning(void);
...
void as3911SetFrameDelayTime(u32 frameDelayTime)
{
	...
	as3911ModifyRegister(AS3911_REG_GPT_CONF, 0xE0, 0x00);   //change according to v1.0.5
...
extern void TimeDelay_us_R(unsigned int Time);
...
void as3911Transmit(const u8 *message, size_t messageLength, AS3911RequestFlags_t requestFlags)
{
    ...
    AS3911RequestFlags_t transmissionModeFlags;
	
	 transmissionModeFlags = requestFlags & AS3911_TX_MODE_MASK;
    ...
    *responseLength = 0;
    do
    {
        ...
        as3911WaitForInterruptTimed(
           ...
           (AS3911_RX_IRQ_SANITY_TIMEOUT+sanity_timeout),
            &irqStatus
            );
        ...
        if (irqStatus & AS3911_IRQ_MASK_WL)
        {
             ...
             */
            as3911ContinuousRead1(AS3911_REG_FIFO_RX_STATUS1, &fifoStatus[0], 2);
			for(i=0;i<2;i++)
			   fifoStatus[i]=buff1[i];
	...
	#ifdef BCTC_DEBUG1
                //SendString2((unsigned char *)"h2");
    #endif
				
                as3911ReadFifo(&response[numBytesReceived], fifoReadSize);
				for(i=0;i<fifoReadSize;i++)
			  			 response[numBytesReceived+i]=buff1[i];
        ...
        if (irqStatus & AS3911_IRQ_MASK_RXE)
        {
            ...
            u8 numBytesInFifo = 0;
            bool reenableReceiver = FALSE;
			
            as3911ContinuousRead1(AS3911_REG_FIFO_RX_STATUS1, &fifoStatus[0], 2);
            numBytesInFifo = buff1[0] & 0x7F;
			for(i=0;i<2;i++)
			   fifoStatus[i]=buff1[i];
            as3911GetInterrupts(
		...
		// print_BYTE(0x99);
            ...
            else
            {
                ...
                else
                {
					...
					for(i=0;i<numBytesInFifo;i++)
			  			 response[numBytesReceived+i]=buff1[i];
                    numBytesReceived += numBytesInFifo;
                }

               
			   	//change according to v1.0.5
			    /* Stop NRT timer. */
...
//				 buff1[299]= numBytesReceived;


			/* 	if((numBytesReceived<4)&&(testboct==11))
				{
					if (errorIrqStatus & AS3911_IRQ_MASK_COL)
                    continue;
    ...
    , u32 timeout, AS3911RequestFlags_t requestFlags)
{
 ...
 //SendString((unsigned char *)"q1 \r\n ");
    ...
    as3911Transmit(request, requestLength, requestFlags);
     ...
     //SendString((unsigned char *)"q3 \r\n ");
     
    returnValue = as3911Receive(response, maxResponseLength, responseLength);
    // print_BYTE(0x66);
    // print_BYTE(returnValue);
	
	if((returnValue==0)&&(debug_flag==1))
	{
		for(i=0;i<(*responseLength-2);i++)
			buf_typeAB[i] = response[i];
		VSAM_len = *responseLength-2;
	}
...
static bool as3911FdtIsRunning()
{
    u8 aux = 0;
    
	//as3911ReadRegister(AS3911_REG_AUX_DISPLAY, &aux);
	as3911ReadRegister(AS3911_REG_VSS_REGULATOR_RESULT, &aux);	//change according to v1.0.5
