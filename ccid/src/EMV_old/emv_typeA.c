...
//#include "../config/iso14443.h"

//#define FALSE  0  //false 
//#define TRUE  1   //true 
//extern unsigned char TypeA_flag ;
...
s16 emvTypeAAnticollision(EmvPicc_t *picc)
{
	...
	while(times<5)
	{
    	error = emvPrelayer4Transceive(NULL, 0, &atqa[0], sizeof(atqa), &responseLength,
				...
				times++;
		if((error==0)&&(responseLength==2)) break;
	}

//	responseLength =buff1[299];
...
s16 emvTypeAActivation(EmvPicc_t *picc)
{
    ...
    u8 *tx_byte = &ats[2];
	    ...
	    EMVSendLength=29;
        emvDisplayString("EMV: TypeA Activation... \r\n");
#endif
	...
	if (error == EMV_ERR_OK)
	{		if(ats[0]>20)  ats[0]=0x20;
			for(i_len=0;i_len<ats[0];i_len++)
			buf_typeAB[i_len]=ats[i_len];
	}
    ...
    if (t0 & 0x20)
    {
        ...
        if(picc->sfgi > EMV_SFGI_MAX_PCD)
            //return EMV_ERR_PROTOCOL;
		    picc->sfgi = EMV_SFGI_DEFAULT;    //AS3911_1526.patch

        if(picc->fwi > EMV_FWI_MAX_PCD)
            //return EMV_ERR_PROTOCOL;
             picc->fwi = EMV_FWI_DEFAULT;    //AS3911_1526.patch
        tx_byte++;
    }
